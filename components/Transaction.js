...
const Transaction = (props) => {
  ...
  const executeTransaction = async (_txId) => {
    try {
      const signer = await getProviderOrSigner(true);
      const walletContract = getWalletContractInstance(signer);
      const txn = await walletContract.execute(_txId, { gasLimit: 3000000 });
      ...
      console.error(error);
      window.alert('This Might be Giving Error Because of Gas Limit and Looking into this issue');
    }
  };
  ...
  const getProviderOrSigner = async (needSigner = false) => {
    ...
    if (chainId !== 280) {
      window.alert("Change the network to Zk sync Network");
      throw new Error("Change network to Zk sync Network");
    }
    if (needSigner) {
  ...
  const handleClick = () => {
    window.open(
      `https://goerli.explorer.zksync.io/address/${props.contractAddress}`
    );
  };
        ...
        <>
          ...
          <Box textAlign="center" justifyContent="center" justify="center">
            <>
              ...
              <Grid sx={{ mb: 3 }}>
                <Link
                  href={`https://goerli.explorer.zksync.io/address/${props.contractAddress}`}
                  underline="hover"
                  target="_blank"
                >
                  View Contract On zk-sync Explorer
                </Link>
              </Grid>
            </>
          </Box>
